var SLStatic = {
  ...
  evaluateUfunc(name, body, prev, args) {
    ...
    try {
      ...
      SLStatic.debug(
        `User function ${name} (prev: ${prev}, args: ${args}) returned error.`,
        ex,
      );
    ...
    switch (recvdType) {
      ...
      case "array":
        ...
        if (typeof response[0] === "number") {
          ...
          let error =
            `Send Later: Array ${response} returned by recurrence function ` +
  ...
  async nextRecurDate(next, recurSpec, now, args) {
    ...
    if (recur.type === "function") {
      ...
      if (recur.until && recur.until.getTime() < nextRecur.sendAt.getTime()) {
        SLStatic.debug(
          `Recurrence ending because of "until" restriction: ${recur.until} < ${nextRecur.sendAt}`,
        );
        return null;
      }
      return nextRecur;
    ...
    if (recur.until && recur.until.getTime() < next.getTime()) {
      SLStatic.debug(
        `Recurrence ending because of "until" restriction: ${recur.until} < ${next}`,
      );
      return null;
    }
...
if (typeof browser === "undefined" && typeof require !== "undefined") {
  ...
  SLStatic.mockStorage.ufuncs = {
    ...
    DaysInARow: {
      help: "Send the message now, and subsequently once per day at the same time, until it has been sent three times. Specify a number as an argument to change the total number of sends.",
      body: "// Send the first message now, subsequent messages once per day.\nif (! prev)\n    next = new Date();\nelse {\n    var now = new Date();\n    next = new Date(prev); // Copy date argument so we don't modify it.\n    do {\n        next.setDate(next.getDate() + 1);\n    } while (next < now);\n    // ^^^ Don't try to send in the past, in case Thunderbird was asleep at\n    // the scheduled send time.\n}\nif (! args) // Send messages three times by default.\n    args = [3];\nnextargs = [args[0] - 1];\n// Recur if we haven't done enough sends yet.\nif (nextargs[0] > 0)\n    nextspec = \"function \" + specname;",
    },
    Delay: {
      help: "Simply delay message by some number of minutes. First argument is taken as the delay time.",
