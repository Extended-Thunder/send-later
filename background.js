...
const SendLater = {
  ...
  async possiblySendMessage(msgHdr) {
    ...
    if (/encrypted/i.test(contentType)) {
      SLStatic.debug(
        `Message ${originalMsgId} is encrypted, and will not be processed by Send Later.`,
      );
      SLTools.unscheduledMsgCache.add(msgHdr.id);
      return;
    }
    ...
    if (msgUUID !== preferences.instanceUUID) {
      SLStatic.debug(
        `Message <${originalMsgId}> is scheduled by a different Thunderbird isntance.`,
      );
      SLTools.unscheduledMsgCache.add(msgHdr.id);
      return;
    }
    ...
    if (lock[msgLockId]) {
      ...
      if (preferences.optOutResendWarning === true) {
        SLStatic.debug(
          `Encountered previously sent message "${msgSubject}" ${msgLockId}.`,
        );
      } else {
        SLStatic.error(
    ...
    if (!(Date.now() >= nextSend.getTime())) {
      SLStatic.debug(
        `Message ${
          msgHdr.id
        } not due for send until ${SLStatic.humanDateTimeFormat(nextSend)}`,
      );
      return;
    }
  ...
  async onUserCommandKeyListener(keyid) {
    ...
    switch (keyid) {
      ...
      case "key_sendLater": {
        ...
        if (SendLater.prefCache.altBinding) {
          SLStatic.info(
            "Passing Ctrl+Shift+Enter along to builtin send later " +
  ...
  async onNewMailReceivedListener(folder, messagelist) {
    if (
      ["sent", "trash", "templates", "archives", "junk", "outbox"].includes(
        folder.type,
      )
    ) {
      SLStatic.debug(
        ...
        await SLTools.forAllDrafts(async (draftHdr) => {
          if (!SLTools.unscheduledMsgCache.has(draftHdr.id)) {
            ...
            if (inReplyTo == `<${draftHdr.headerMessageId}>`) {
              let cancelOnReply = await messenger.messages
                .getFull(draftHdr.id)
                .then(
                  (draftMsg) =>
  ...
  async onMessageDisplayedListener(tab, hdr) {
    ...
    if (hdr.folder.type == "drafts") {
      ...
      SLStatic.debug(
        "This is not a Drafts folder, so Send Later will not scan it.",
      );
    }
  },
  ...
  async disable() {
    ...
    await SLStatic.nofail(
      messenger.headerView.onHeaderRowUpdate.removeListener,
      SendLater.headerRowUpdateListener,
    );
    await SLStatic.nofail(
