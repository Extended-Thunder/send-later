...
const SendLaterFunctions = {
  ...
  queueSendUnsentMessages() {
    if (Utils.isOffline) {
      SendLaterFunctions.debug(
        "SendLaterFunctions.queueSendUnsentMessages Deferring sendUnsentMessages while offline",
      );
    } else if (SendLaterVars.sendingUnsentMessages) {
      SendLaterFunctions.debug(
        "SendLaterFunctions.queueSendUnsentMessages Deferring sendUnsentMessages",
      );
  ...
  sendUnsentMessagesListener: {
    ...
    onStartSending: function (aTotalMessageCount) {
      SendLaterFunctions.debug(
        "Entering function: SendLaterFunctions.sendUnsentMessagesListener.onStartSending",
      );
      ...
      SendLaterFunctions.debug(
        "Leaving function: SendLaterFunctions.sendUnsentMessagesListener.onStartSending",
      );
    },
    ...
    onStopSending: function (aStatus, aMsg, aTotalTried, aSuccessful) {
      SendLaterFunctions.debug(
        "Entering function: SendLaterFunctions.sendUnsentMessagesListener.onStopSending",
      );
      ...
      SendLaterFunctions.debug(
        "Leaving function: SendLaterFunctions.sendUnsentMessagesListener.onStopSending",
      );
    },
  },
          ...
          return new Promise((resolve, reject) => {
            ...
            const DummyCompleteGenericSendMessage = (detail) => {
              ...
              if (detail === Ci.nsIMsgCompDeliverMode.Later) resolve(true);
                ...
                reject(
                  "Something strange happened while performing pre-send checks.",
                );
            };
