...
function init() {
  ...
  document
    ...
    .addEventListener("click", () => {
      messenger.runtime
        .sendMessage({ action: "contactAuthor" })
        .then(() => window.close());
    });
  document.getElementById("donateButton").addEventListener("click", () => {
    messenger.runtime
      .sendMessage({ action: "donateLink" })
      .then(() => window.close());
  });
